...
try:
    conn = mysql.connector.connect(host="localhost", user="root",
                                   password="Gurumysql@4227", database="onehealth", auth_plugin='mysql_native_password')
except:
    print("error")
...
def ValuePredictor(to_predict_list, size):
    ...
    if(size == 6):
        loaded_model = joblib.load(
            r'D:\OneHealth\OneHealth\diabetes_model.pkl')
...
def ValuePredictorheart(to_predict_list, size):
    ...
    if(size == 7):
        loaded_model = joblib.load(
            r'D:\OneHealth\OneHealth\heart_model.pkl')
        result = loaded_model.predict(to_predict)
    return result[0]
